...
- [ ] T001 Aucune nouvelle dépendance/configuration nécessaire (feature de lecture pure sur schéma
  existant) — phase de setup vide, passer directement à la Phase 2.

## Phase 2: Foundational (bloquant pour toutes les User Stories)

- [ ] T002 [P] Créer `src/KekeBeauty.Application/Directory/Dtos.cs` : classes `EtablissementSummary`,
  `MediaDto`, `PrestationDto`, `EtablissementDetail` (voir `data-model.md`)
- [ ] T003 [P] Créer `src/KekeBeauty.Application/Directory/IDirectoryRepository.cs` (recherche par
  catégorie/commune, détail par id — tous deux filtrés sur `statut_kyc = 'VALIDE'`)
- [ ] T004 [P] Créer `src/KekeBeauty.Application/Directory/ICategorieRepository.cs`
  (`FindOrCreateByLibelleAsync`)
- [ ] T005 [P] Créer `src/KekeBeauty.Application/Directory/IPrestationRepository.cs` (`AddAsync`)
- [ ] T006 Créer `src/KekeBeauty.Infrastructure/Directory/DirectoryRepository.cs` (Dapper, `INNER JOIN
  etablissement_categorie` — voir `research.md` Décision 1)
- [ ] T007 [P] Créer `src/KekeBeauty.Infrastructure/Directory/CategorieRepository.cs` (Dapper,
  `ON CONFLICT (libelle_categorie) DO NOTHING`)
- [ ] T008 [P] Créer `src/KekeBeauty.Infrastructure/Directory/PrestationRepository.cs` (Dapper, `INSERT`)
...
- [ ] T009 [US4] Créer `src/KekeBeauty.Application/Directory/AssignCategoryUseCase.cs`
- [ ] T010 [US4] Créer `src/KekeBeauty.Application/Directory/AddPrestationUseCase.cs`
- [ ] T011 [US4] Étendre `src/KekeBeauty.Api/Controllers/AdminController.cs` : `POST
...
- [ ] T012 [US4] Enregistrer les nouveaux services dans le DI (`Program.cs`)
- [ ] T013 [US4] Exécuter le Scénario 1 de `quickstart.md` sur l'établissement validé de la feature 004
...
- [ ] T014 [US1] Créer `src/KekeBeauty.Application/Directory/SearchEtablissementsUseCase.cs`
- [ ] T015 [US1] Créer `src/KekeBeauty.Api/Controllers/EtablissementsController.cs` : `GET
  /etablissements?categorie=&commune=` (public, mappé selon `contracts/directory-api.md`)
- [ ] T016 [US1] Exécuter le Scénario 2 de `quickstart.md` (résultat non vide puis liste vide)
...
- [ ] T017 [US2] Exécuter le Scénario 3 de `quickstart.md` (soumission 004 + assignation catégorie
...
- [ ] T018 [US3] Créer `src/KekeBeauty.Application/Directory/GetEtablissementDetailUseCase.cs`
  (génère `lienItineraire`, voir `research.md` Décision 5)
- [ ] T019 [US3] Ajouter `GET /etablissements/{id}` à `EtablissementsController.cs` (404 uniforme,
  `research.md` Décision 2)
- [ ] T020 [US3] Exécuter le Scénario 4 de `quickstart.md` (fiche complète)
- [ ] T021 [US3] Exécuter le Scénario 5 de `quickstart.md` (404 identique non-validé/inexistant)

**Checkpoint**: US1 + US2 + US3 + US4 livrées — feature complète.

## Phase 7: Polish & Cross-Cutting Concerns

- [ ] T022 [P] Mettre à jour `README.md` (section API backend) : endpoints de recherche/fiche,
  note sur le mécanisme FR-008 temporaire
- [ ] T023 Mettre à jour `docs/scrum/product-backlog.md` (US-06 à US-09 → `005-recherche-annuaire`)
