...
public sealed class AdminAuthController(IAdminManagementRepository repo,AdminSessionTokenService tokens,IConfiguration config):ControllerBase {
 ...
 private async Task EnsureBootstrapAsync(CancellationToken ct){if(await repo.CountAdminsAsync(ct)>0)return;var email=config["Admin:BootstrapEmail"]??"admin@kekebeauty.ci";var pass=config["Admin:BootstrapPassword"]??config["Admin:ApiKey"];if(AdminPasswordPolicy.IsValid(pass))await repo.CreateAdminAsync("Administrateur Keke Beauty",email,Hash(pass!),"SUPER_ADMIN",ct);}
